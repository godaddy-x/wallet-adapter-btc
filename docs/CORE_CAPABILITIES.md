...
| Scanner node `SpeedUpFromOriginTrade` / `CancelFromOriginTrade` | ✅ (DB snapshot → `createRawTransaction(..., speedUp)`, replaceable txid from submit chain) |
| Platform SpeedUp/Cancel APIs | ✅ (EVM today; calls scanner RPC) |
